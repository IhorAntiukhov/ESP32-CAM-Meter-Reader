...
bool ClassFlowPostProcessing::doFlow(string zwtime)
{
    ...
    for (int j = 0; j < NUMBERS.size(); ++j)
    {
        ...
        if (!NUMBERS[j]->AllowNegativeRates)
        {
            ...
            {
                NUMBERS[j]->ErrorMessageText = NUMBERS[j]->ErrorMessageText + "Отрицательное изменение - " + zwvalue + " Сырое значение: " + NUMBERS[j]->ReturnRawValue + " Предвар. значение: " + RundeOutput(NUMBERS[j]->PreValue, NUMBERS[j]->Nachkomma) + " "; 
        ...
        if (NUMBERS[j]->useMaxRateValue && PreValueUse && NUMBERS[j]->PreValueOkay)
        {
            ...
            {
                NUMBERS[j]->ErrorMessageText = NUMBERS[j]->ErrorMessageText + "Изменение показания слишком большое - " + RundeOutput(NUMBERS[j]->Value, NUMBERS[j]->Nachkomma) + " Предвар. значение: " + RundeOutput(NUMBERS[j]->PreValue, NUMBERS[j]->Nachkomma);
                NUMBERS[j]->Value = NUMBERS[j]->PreValue;
                NUMBERS[j]->ReturnValue = "";
                NUMBERS[j]->ReturnRateValue = "";
                continue;
        ...
        NUMBERS[j]->ErrorMessageText = "Ошибок нет";
        UpdatePreValueINI = true;

        string _zw = "Постобработка - Сырое значение: " + NUMBERS[j]->ReturnRawValue + " Показание: " + NUMBERS[j]->ReturnValue + " Ошибка: " + NUMBERS[j]->ErrorMessageText;
