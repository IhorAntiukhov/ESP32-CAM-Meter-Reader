...
//    printf("_stepname: %s\n", _stepname.c_str());
    ...
    if ((_stepname.compare("[Firebase]") == 0) || (_stepname.compare(";[Firebase]") == 0)){
        _classname = "ClassFlowFirebase";
    }
...
std::string ClassFlowControll::TranslateAktstatus(std::string _input)
{
    if (_input.compare("ClassFlowMakeImage") == 0)
        return ("делаем фотографию");
    if (_input.compare("ClassFlowAlignment") == 0)
        return ("выравниваем изображение");
    if (_input.compare("ClassFlowAnalog") == 0)
        return ("распознавание аналоговых счётчиков");
    if (_input.compare("ClassFlowCNNGeneral") == 0)
        return ("распознавание цифрового счётчика");
    if (_input.compare("ClassFlowFirebase") == 0)
        return ("отправляем показания на Firebase");
...
ClassFlow* ClassFlowControll::CreateClassFlow(std::string _type)
{
    ...
    if (toUpper(_type).compare("[FIREBASE]") == 0)
        cfc = new ClassFlowFirebase(&FlowControll);
...
//        aktstatus = zw_time + ": " + FlowControll[i]->name();
    ...
    zw_time = gettimestring("%H:%M:%S");
    aktstatus = "распознавание завершено (" + zw_time + ")";
    return result;
}
                ...
                case READOUT_TYPE_PREVALUE:
                    if (flowpostprocessing->PreValueUse)
                    {
                        if ((*numbers)[i]->PreValueOkay)
                            ...
                            out = out + "Предварительное значение устарело";                
                    }
                    else
                        out = out + "Предварительное значение отключено";
                    break;
...
//    printf("Input UpdatePreValue: %s\n", _newvalue.c_str());
    ...
    else
    {
        ...
        if (zw == 0)
            return "- Ошибка преобразования строки в число! Строка должна иметь формат 123.456";
    }
...
string ClassFlowControll::getJSON()
{
    ...
    for (int i = 0; i < (*NUMBERS).size(); ++i)
    {
        ...
        if ((*NUMBERS)[i]->ReturnValue.length() > 0)
            json += "    \"Значение\": "      + (*NUMBERS)[i]->ReturnValue          + ",\n";
        else
            json += "    \"Значение\": \"\",\n";
        json += "    \"Сырое значение\": \""        + (*NUMBERS)[i]->ReturnRawValue              + "\",\n";
        json += "    \"Ошибка\": \""     + (*NUMBERS)[i]->ErrorMessageText             + "\",\n";
        if ((*NUMBERS)[i]->ReturnRateValue.length() > 0)
            json += "    \"Изменение\": "      + (*NUMBERS)[i]->ReturnRateValue                + ",\n";
        else
            json += "    \"Изменение\": \"\",\n";

        json += "    \"Время\": \"" + (*NUMBERS)[i]->timeStamp                    + "\"\n";
        if ((i+1) < (*NUMBERS).size())
            json += "  },\n";
        else
            json += "  }\n";
